...
var Y = require('yjs');
var func = require('lib0/function');
  ...
  binding._mux(() => {
    binding.doc.transact(() => {
      ...
      if (changes.length > 1) {
        ...
        const d = diff__namespace.simpleDiffString(
        ...
        const delLen = change.removed.map((s) => s.length).reduce(math__namespace.add) +
...
const updateRemoteSelection = (y, cm, type, cursors, clientId, awareness) => {
  ...
  if (
    ...
    if (
      m && func__namespace.equalityFlat(aw.cursor.anchor, m.awCursor.anchor) &&
      func__namespace.equalityFlat(aw.cursor.head, m.awCursor.head)
    ) {
    ...
    if (head.index !== anchor.index) {
      if (from.line !== to.line && from.ch !== 0) {
        ...
        sel.push(
          cm.markText(from, new CodeMirror__default["default"].Pos(from.line + 1, 0), {
      ...
      while (from.line !== to.line) {
        ...
        sel.push(
          cm.setBookmark(new CodeMirror__default["default"].Pos(from.line, 0), {
...
const codemirrorCursorActivity = (y, cm, type, awareness) => {
  ...
  const newAnchor = Y__namespace.createRelativePositionFromTypeIndex(
    type,
    cm.indexFromPos(cm.getCursor('anchor'))
  );
  const newHead = Y__namespace.createRelativePositionFromTypeIndex(
    type,
  ...
  if (aw.cursor != null) {
    currentAnchor = Y__namespace.createRelativePositionFromJSON(
  ...
  if (
    aw.cursor == null ||
    !Y__namespace.compareRelativePositions(currentAnchor, newAnchor) ||
    !Y__namespace.compareRelativePositions(currentHead, newHead)
  ) {
 ...
 */
class CodemirrorBinding extends observable.Observable {
  /**
  ...
  constructor (
    ...
    this._onStackItemPopped = ({ stackItem }) => {
      ...
      if (sel) {
        const anchor =
          Y__namespace.createAbsolutePositionFromRelativePosition(sel.anchor, doc).index;
        const head =
          Y__namespace.createAbsolutePositionFromRelativePosition(sel.head, doc).index;
    ...
    this._beforeChange = () => {
      ...
      this._mux(() => {
        ...
        const anchor = Y__namespace.createRelativePositionFromTypeIndex(
          textType,
          codeMirror.indexFromPos(codeMirror.getCursor('anchor'))
        );
        const head = Y__namespace.createRelativePositionFromTypeIndex(
          textType,
